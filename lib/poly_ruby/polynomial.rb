...
def Polynomial(poly_arg1 = [0], one = 1)
  case poly_arg1
  when Polynomial; poly_arg1.clone
  when Array; Polynomial.new(poly_arg1, one).normalize!
  when Numeric; Polynomial.new(poly_arg1, one)
  when String
...
class Polynomial
  ...
  def inspect
    s = ""
    print "["
    @array.each_index { |k| print s + k.to_s
                            s = "," }
    print "]\n"
  end
  ...
  def []=(i, c)
    l0 = @array.size
    if l0 < i
      @array.fill(@zero, l0..i)
    end
    @array[i] = c
  end

  def clone
    f = Polynomial.new([@zero], @one)
    f.array.replace(@array)
    f
  end
  ...
  def lp # leading power product
    Polynomial.term(@one, self.degree, @one)
  end

  def lt # leading term
    Polynomial.term(@array[-1], self.degree, @one)
  end
  ...
  def zero?
    (@array[0] == @zero) && (self.degree == 0)
  end

  def unit?
    if @array.size != 1; return false; end
    lc = @array[0]
    (lc == @one) || (lc == -@one) || (lc.kind_of?(Rational)) || (lc.kind_of?(Float))
  end
  ...
  def timesSep(p2, d1, d2)
    ...
    p1a = Polynomial.new(@zero, @one)
    p1a.array = @array[d + 1..d1]
    d1a = d1 - d - 1
    p1b = Polynomial.new(@zero, @one)
    p1b.array = @array[0..d]
    p1b.normalize!
    d1b = p1b.degree
    p2a = Polynomial.new(@zero, @one)
    p2a.array = p2.array[d + 1..d2]
    d2a = d2 - d - 1
    p2b = Polynomial.new(@zero, @one)
    p2b.array = p2.array[0..d]
    p2b.normalize!
    d2b = p2b.degree
    if d2a >= d00
      aa = p1a.timesSep(p2a, d1a, d2a)
    else aa = p1a.timesCnv(p2a)
    end
    if (d1b >= d2b)
      if (d2b >= d00)
        bb = p1b.timesSep(p2b, d1b, d2b)
      else bb = p1b.timesCnv(p2b)
      end
    else if (d1b >= d00)
           bb = p2b.timesSep(p1b, d2b, d1b)
    else bb = p1b.timesCnv(p2b)
    end
  end
    if d1b >= d1a
      p1c = p1b
      0.upto(d1a) { |i| p1c.array[i] += p1a.array[i] } else p1c = p1a
                                                            0.upto(d1b) { |i| p1c.array[i] += p1b.array[i] } end
    if d2b >= d2a
      p2c = p2b
      0.upto(d2a) { |i| p2c.array[i] += p2a.array[i] } else p2c = p2a
                                                            0.upto(d2b) { |i| p2c.array[i] += p2b.array[i] } end
    d1c = p1c.degree
    d2c = p2c.degree
    if (d1c >= d2c)
      if (d2c >= d00)
        ab = p1c.timesSep(p2c, d1c, d2c)
      else ab = p1c.timesCnv(p2c)
      end
    else if (d1c >= d00)
           ab = p2c.timesSep(p1c, d2c, d1c)
    else ab = p1c.timesCnv(p2c)
    end end
    ab = ab - (aa + bb)
  ...
  def *(other)
    if other.kind_of?(Polynomial)
      ...
      if (d1 >= d2)
        if (d2 >= d00)
          p = self.timesSep(other, d1, d2)
        else p = self.timesCnv(other)
        end else if (d1 >= d00)
                   p = other.timesSep(self, d2, d1)
      else p = self.timesCnv(other)
      end end
      p
  elsif other.kind_of?(@one.class) or other.kind_of?(Numeric)
    d = self.degree
    a = Polynomial.new(@one, @one)
    a.array = Array.new(d + 1)
  ...
  def powerModI(power, m, n = 0)
    ...
    if power.kind_of?(Integer) and power >= 0
      if n == 0
        ...
        # calculate ** following to binary notation of "power".
        p = self.modI(m).coeff_to_Zp(n)
        s = Polynomial.new([@one], @one)
        while power > 0
          if power & 1 == 1; s = (s * p).modI(m).coeff_to_Zp(n); end
          p = (p * p).modI(m).coeff_to_Zp(n)
          power >>= 1
        end
        s
      end
  ...
  def divmod(divisor) # assume that coefficient is field.
    if divisor.kind_of?(Polynomial)
      r = self.clone
      degR = r.degree # Remainder(dividend)
      degD = divisor.degree
      topD = divisor.array[degD]
    # Divisor
      degQ = degR - degD
      if degQ < 0
        degQ = 0
      end
      q = Polynomial.new(@zero, @one)
      q.array[0..degQ] = @zero #Quotient
      while degR >= degD
        if r.array[degR] == @zero; q.array[degR - degD] = @zero else
                                                                  q1 = Number.divII(r.array[degR], topD)
                                                                  dq = degR - degD
                                                                  q.array[dq] = q1
                                                                  for i in (0..degD - 1)
                                                                    r.array[i + dq] -= divisor.array[i] * q1; end
      ...
      [q.normalize!, Polynomial(@zero, @one)]
    else
      self.divmod(Polynomial(divisor))
    end
  end
  ...
  def divmod_i(divisor)
    ...
    if divisor.kind_of?(Polynomial)
      ...
      degR = r.degree # Remainder(dividend)
      degD = divisor.degree
      topD = divisor.array[degD] # Divisor
      degQ = degR - degD
      if degQ < 0
        degQ = 0
      end
      ...
      self.divmod_i(Polynomial(divisor))
    end
  end

  def modZp(divisor, p)
    q, r = self.divmodZp(divisor, p)
    r
  end
  ...
  def divmodZp(divisor, p)
    ...
    if divisor.kind_of?(Polynomial)
      ...
      degD = dv.degree
      topDr = Number.inv(dv.array[degD], p) # Divisor
      degQ = degR - degD
      if degQ < 0
        degQ = 0
      end
      q = Polynomial.new
      q.array[0..degQ] = 0 #Quotient
      while degR >= degD
        if r.array[degR] == 0; q.array[degR - degD] = 0 else
                                                          ...
                                                          if q1 != 0
                                                            q1 = (p - q1) % p
                                                            for i in (0..degD - 1)
                                                              r.array[i + dq] += dv.array[i] * q1; end
                                                          end
        end
        r.array[degR] = 0
        degR -= 1
      end
      [q.normalize!.coeff_to_Zp(p), r.normalize!.coeff_to_Zp(p)]
    else
      self.divmodZp(Polynomial(divisor), p)
    end
  end
  ...
  def derivative(n = 1)
    p = self.clone
    p = p.normalize!
    n.times { p.array.each_index { |i| p.array[i] *= i }
              p.array.shift }
    p.normalize!
  end
  ...
  def coerce(x)
    case x
    when Numeric; [Polynomial(x), self]
    when String; [Polynomial(x), self]
    when Array; [Polynomial(x), self]
    else; raise TypeError     end
  end
  ...
  def gcd_coeff_num # gcd of numerator coefficients as Rational
    ...
    if num.empty?; 1; else Number.gcd(num); end
  end
  ...
  def coeff_truncate # truncate each coefficient to Integer
    f = self.clone
    f.array.each_with_index { |x, i| f.array[i] = x.truncate }
    f
  end

  def coeff_round # round each coefficient to Integer
    f = self.clone
    f.array.each_with_index { |x, i| f.array[i] = x.round }
    f
  end

  def coeff_to_f # converts each element to Float
    f = self.clone
    f.array.each_with_index { |x, i| f.array[i] = x.to_f }
    f
  end
  ...
  def to_s(format = "text", v = "x", r = true)
    ...
    for i in (0..deg); if r; d = deg - i; else d = i; end
                      ...
                      s = s + addS
                         ...
                         if d != 0; s = s + v
                                    if d != 1; s = s + power1 + (d.to_s) + power2; end       end
                         s = s + den
                       end     end
    if s == ""; s = "0"; end
  ...
  def Polynomial.gcd2sZp(prime, a, b)
    ...
    while true
      if b.zero?; return a, x, y; end
      q, a = a.divmodZp(b, prime)
      x = (x - q * u).coeff_to_Zp(prime)
      y = (y - q * v).coeff_to_Zp(prime)
      if a.zero?; return b, u, v; end
      q, b = b.divmodZp(a, prime)
      u = (u - q * x).coeff_to_Zp(prime)
      v = (v - q * y).coeff_to_Zp(prime)
    end
  end
  ...
  def getSturm # return  GCD, Sturm_sequence
    sturm = []
    sturm.push(self.clone)
    sturm.push(self.derivative)
  ...
  def countSturmChange(a, sturm)
    sturmA = []
    sturm.each { |p|
      if a == Infinity; sturmA.push(p.lc) elsif a == -Infinity
                                            if p.degree & 1 == 1; sturmA.push(-p.lc); else sturmA.push(p.lc); end
      else pa = p.substitute(a)
           if pa != @zero
             sturmA.push(pa)
           end
         end
  ...
  def countSolution(a = -Infinity, b = Infinity, countRedundancy = true)
    ...
    if (a == Infinity) || (b == -Infinity); return 0 elsif (a == -Infinity) || (b == Infinity)
    elsif a.kind_of?(Numeric) && b.kind_of?(Numeric) && (a >= b); return 0     end
    if a.kind_of?(Numeric) and substitute(a) == 0
      return (self / Polynomial.new([-a, 1], 1)).countSolution(a, b, countRedundancy)
    end
    if b.kind_of?(Numeric) and substitute(b) == 0
      return (self / Polynomial.new([-b, 1], 1)).countSolution(a, b, countRedundancy)
    end
    gcd_s, sturm = self.getSturm
    if (countRedundancy) && (gcd_s.degree > 0); n1 = gcd_s.countSolution(a, b); else n1 = 0; end
    countSturmChange(a, sturm) - countSturmChange(b, sturm) + n1
  end
  ...
  def squareFreeDecomposition
    ...
    while g[-1].degree > 0
      gm = g[-1].clone
      g.push(Polynomial.gcd(gm, gm.derivative))
   end
    h = [Poly(1)]
    i = 1
    while i < g.size
  ...
  def SqrFree?(prime = 0)
    if prime == 0
      gcd = Polynomial.gcd(self, self.derivative)
    else
      gcd = Polynomial.gcdZp(prime, self, self.derivative)
    end
    (gcd.degree == 0)
  end
  ...
  def factorize
    Factorization.factorize(self)
  end

  def Polynomial.factor2s(f, s = "")
    if s == ""; "(" + Number.factor2s(f, ")(") + ")" else Number.factor2s(f, s) end
  end
  ...
  module Factorization
    ...
    def setV0Tbl(degD) # table of PolyN.
      ...
      V0.clear
      for i in (0..degD)
        V0[i] = PolyN.substitute(i - (degD.div(2))).abs
      end
    end
    ...
    def setV1Tbl(degD) # table of DpolyMs
      V1.clear
      for i in (0..degD)
        V1[i] = DpolyMs.substitute(i - (degD.div(2)))
      end
    end
    ...
    def solveQ1(q, prime)
      ...
      for i in 0..n - 1
        # set pivot
        j1 = 0
        while (j1 < n) && ((q[i][j1] == 0) || (pivot[j1] != -1))
          j1 = j1 + 1
        end
        if j1 < n
    ...
    def setPolyB(depth, dpoly, prime)
      if depth < FactorP.size
        ...
        if depth == 0
          if PolyN.degree > 0
            Factor.push(PolyN.clone)
            PolyN.array.replace([1])
          end
          true
        else
          dpoly = dpoly * FactorP[depth]
          if setPolyB(depth + 1, dpoly, prime)
            FactorP.delete_at(depth)
            return true
          end
          false
        end
        ...
        if dpoly.degree > 0
          ...
          resume(prime) # TRUE if find Factors
        end
      end
    end
    ...
    def checkZp(i, prime)
      ...
      checkDivZp(PolyM, DpolyM, prime) && resume(prime)
    end
    ...
    def factorizeSqrFree(p)
      ...
      if PolyN.degree < 11
        ...
        else setPolyBerlekamp(prime)         end
      end
    ...
    def factorize(p) # wrapper of factorizeSqrFree(p)
      ...
      sqrF.each_with_index { |f, i|
        if f.degree > 0; ff = factorizeSqrFree(f)
                         i.times { ff.each { |ffc| factorA.push(ffc.clone) } };         end
      }
