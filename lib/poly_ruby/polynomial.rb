...
class Polynomial
  ...
  def timesSep(p2, d1, d2)
    ...
    if (d1b >= d2b); if (d2b >= d00); bb = p1b.timesSep(p2b, d1b, d2b); else bb = p1b.timesCnv(p2b); end; else if (d1b >= d00); bb = p2b.timesSep(p1b, d2b, d1b); else bb = p1b.timesCnv(p2b); end; end
    if d1b >= d1a; p1c = p1b; 0.upto(d1a) { |i| p1c.array[i] += p1a.array[i] } else p1c = p1a; 0.upto(d1b) { |i| p1c.array[i] += p1b.array[i] } end
    if d2b >= d2a; p2c = p2b; 0.upto(d2a) { |i| p2c.array[i] += p2a.array[i] } else p2c = p2a; 0.upto(d2b) { |i| p2c.array[i] += p2b.array[i] } end
    d1c = p1c.degree; d2c = p2c.degree
    if (d1c >= d2c); if (d2c >= d00); ab = p1c.timesSep(p2c, d1c, d2c); else ab = p1c.timesCnv(p2c); end; else if (d1c >= d00); ab = p2c.timesSep(p1c, d2c, d1c); else ab = p1c.timesCnv(p2c); end end
    ab = ab - (aa + bb)
  ...
  def *(other)
    if other.kind_of?(Polynomial)
      ...
      if (d1 >= d2); if (d2 >= d00); p = self.timesSep(other, d1, d2); else p = self.timesCnv(other); end else if (d1 >= d00); p = other.timesSep(self, d2, d1); else p = self.timesCnv(other); end end
      return p
    elsif other.kind_of?(@one.class) or other.kind_of?(Numeric)
  ...
  def countSturmChange(a, sturm)
    ...
    sturm.each { |p|
      ...
      else pa = p.substitute(a); if pa != @zero; sturmA.push(pa); end;       end
    }
    v = 0 #  #of sign change
  ...
  def SqrFree?(prime = 0)
    if prime == 0; gcd = Polynomial.gcd(self, self.derivative) else gcd = Polynomial.gcdZp(prime, self, self.derivative) end
    return (gcd.degree == 0)
  end
  ...
  def Polynomial.factor2s(f, s = "")
    if s == ""; return "(" + Number.factor2s(f, ")(") + ")" else return Number.factor2s(f, s) end
  end
  ...
  module Factorization
    ...
    def setVal(ite, degD, prime)
      ...
      while ((-v0i - v1i).div(prime) <= a) && (a <= (v0i - v1i).div(prime))
        ...
        if a <= 0; a = 1 - a; if a > (v0i - v1i).div(prime); a = -a; end else a = -a; if a < (-v0i - v1i).div(prime); a = 1 - a; end end
      end
      return false
    end
      ...
      while i < gcdList1.size - 1
        ...
        while j < gcdList1.size
          ...
          if r.zero?; changeFlg = true
            if q.degree > 0; gcdList1[j] = q; j = j + 1 else gcdList1.delete_at(j); end           else j = j + 1           end
        end
    ...
    def factorizeSqrFree(p)
      ...
      if degN == 1; Factor.push(PolyN); return Factor elsif degN == 0; return Factor end
