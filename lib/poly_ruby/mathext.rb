...
class Rational
  ...
  def approximate(accMin = AccuracyMin, accMax = AccuracyMax)
    ...
    while ((n > accMax) || (d > accMax)) && (n > accMin) && (d > accMin)
      n = n / 2
      d = d / 2
 end
    if s == -1; n = -n; end
  ...
  def to_f_str(accuracy = 17)
    ...
    a = [[n.to_s.size, d.to_s.size].min, accuracy].max
    sw = false
    p, q = n.divmod(d)
    str = p.to_s
    n = q * 10
...
module MathExt
  ...
  def sin(a)
    if a.kind_of?(Numeric)
      ...
      if x > PIh; x = PI - x; end
      if x <= PIq
        z = x * SinApprox.substitute(x * x) else y = PIh - x
                                                 z = CosApprox.substitute(y * y) end
      if sgn < 0; -z; else z; end
    else a * SinApprox.substitute(a * a)     end
  end

  def cos(a)
    if a.kind_of?(Numeric); sin(a + PIh) else CosApprox(a * a) end
  end
  ...
  def exp(a)
    if a.kind_of?(Numeric)
      ...
      if sgn < 0; 1 / z; else z; end
    else
      ExpApprox.substitute(a)
    end
  end
  ...
  def sqrt(a)
    if a.kind_of?(Numeric)
      ...
      if shift >= 0; z * (2 ** shift); else z / (2 ** (-shift)); end
    elsif a.kind_of?(Polynomial)
  ...
  def gamma(a)
    if a.kind_of?(Integer)
      if a <= 0; Number.NaN_IEEE754; else Number.factorial(a); end
    elsif a.kind_of?(Float) or a.kind_of?(Rational)
      if a.kind_of?(Rational); x = a.to_f; else x = a; end
      l = 10
      f1 = 1
      while x < l
        f1 = f1 * x
  ...
  def beta(z, w)
    exp(lgamma(z) + lgamma(w) - lgamma(z + w))
  end
