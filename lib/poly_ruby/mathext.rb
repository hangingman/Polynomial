...
module MathExt
  ...
  def sin(a)
    if a.kind_of?(Numeric)
      ...
      if x > PIh; x = PI - x; end
      if x <= PIq; z = x * SinApprox.substitute(x * x) else y = PIh - x; z = CosApprox.substitute(y * y) end
      if sgn < 0; return -z; else return z; end
    else return a * SinApprox.substitute(a * a)     end
  end

  def cos(a)
    if a.kind_of?(Numeric); return sin(a + PIh) else return CosApprox(a * a) end
  end
  ...
  def atan2(y, x) # return (-PI, PI]
    ...
    if x1 > 0; x2 = x1
      if y1 >= 0; quadrant = 1; y2 = y1; else quadrant = 4; y2 = -y1; end     elsif x1 < 0; x2 = -x1
      if y1 >= 0; quadrant = 2; y2 = y1; else quadrant = 3; y2 = -y1; end     else
