...
class Complex
  ...
  def divmodE(other); Number.divmodC(self, other); end
  def modE(other)
    q, r = Number.divmodC(self, other)
    r
 end
end
...
module Number
  ...
  def rmodP(n, p, fold = false) # 0<= modP <p, if fold, -p/2 < modP <= p/2
    if n.kind_of?(Rational)
      d = n.denominator
      n = n.numerator * inv(d, p)
 end
    modP(n, p, fold)
  end
  ...
  def divII(a, b) # return a/b
    if a.kind_of?(Integer) && b.kind_of?(Integer)
      q = a.div(b)
      if a == (b * q); return q; elsif defined? Rational; return Rational(a, b); else; return a.to_f / b.to_f; end
    else
  ...
  def sqrti(n, p = 0)
    if p == 0 # Integer s.t. i^2<=n<(i+1)^2
      ...
      loop {
        ...
        if m == r; return -1; end
        t = y % p
        (r - m - 1).times { t = (t * t) % p } #t=y^(2^(r-m-1))
        y = (t * t) % p
        r = m % p
        x = (x * t) % p
        b = (b * y) % p
  ...
  def rabin_miller?(n) # Rabin-Miller pseudo prime
    ...
    p = 2
    20.times {
      if n == p; return true; elsif n % p == 0; return false; elsif not Number.spsp_o?(n, q, t, p); return false; end
  ...
  def makePrimeList2(n = 3)
    ...
    primeMax = nextPrime(primeMax)
    PrimeList.push(primeMax)
  end
  ...
  def makePrimeList3(n = 3)
    ...
    ns = n - shift
    sieveErato = []
    sieveErato.fill(0, 0..ns) # Note that sieveErato[] is shifted
    PrimeList.each { |p|
      if p * p > n; break; end
      if p % 2 == 0
        i = (primeMax + 1)
        step = 2 # Assume primeMax be odd.
      else
        i = p * p
        step = p + p
        if i < primeMax
          i = i + ((primeMax - i).div(step) + 1) * step
        end
      end
      i = i - shift
      while i <= ns
  ...
  def makePrimeList4(n = 3)
    ...
    ns = (n - shift).div(2)
    sieveErato = []
    sieveErato.fill(0, 0..ns) # shifted and abbrevated
    PrimeList.each { |p|
      if p * p > n; break; end
      if p % 2 == 1
        i = p * p
  ...
  def primitive_root(p) # primitive root modulo p.
    ...
    for a in 2..p - 1
      f = true
      primes.each { |q| if 1 == (a ** ((p - 1) / q) % p) then f = false
                                                              break
 end }
  ...
  def j_delta(r)
    q = Math.exp(r * 2 * MathExt::pi * Complex::I)
    d = Complex.new(0.0)
    n = 1; loop {
  ...
  def j_invariant(r) # r=a+bi
    # j(r)=j(Er). Er=C/Lr and Lr is the lattice generated by 1 and r.
    # modular invariant of elliptic curve.
    f = j_delta(r * 2) / j_delta(r)
    s = Number.powerI(256 * f + 1, 3) / f
    s.real.round
  end
...
=end
    ...
    loop {
      ...
      loop {
        ...
        if t % a == 0
          ...
          if (a == b) or (a * a == t) or (b == 0); p = (x - j) * p; else p = (x * x - 2 * j.real * x + j.abs2) end
        end
        #4
        a += 1
        if a * a > t; break; end
  ...
  def factorize(n)
    ...
    r1 = 5
    r2 = 7 # r=\pm 1 (mod 6). So, step s is \pm 2(mod 6).
    d = sqrti(n)
    while (r1 <= d)
      if n % r1 == 0
        while n % r1 == 0
  ...
  def getDivisors(n)
    ...
    while i < factors.size
      if factors[i] == factors[i + 1]
        factord[i] += 1
        factors.delete_at(i + 1)
 else i = i + 1
      factord[i] = 1 end
    end
    getDivisorsS(0, 1, factors, factord)
  end
  ...
  def comb(m, n) # combination
    (factorial(m).div(factorial(n))).div(factorial(m - n))
  end

  def perm(m, n) # permutation
    factorial(m).div(factorial(m - n))
  end
  ...
  NotationCharRH = {}
  NotationCharA.each_with_index { |c, i| NotationCharRH[c] = i }
  ...
  def i_to_notation_str(i, b)
    str = ""
    i_to_notation_array(i, b).each { |v| str = NotationCharA[v] + str }
    str
  end
